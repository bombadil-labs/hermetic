...
That shared object had the same shape at first, an object literal with getters, and V8 kept it in dictionary mode too, so every read through it was a lookup in a table. It is now an instance of a class, whose getters V8 keeps in fast mode and can inline. In a run that timed both versions of the lifted source, Schema decoding went from 76% slower than the original to 20% slower, and the collections workload from 63% to 55%. `npm run corpus -- bench --literal` reproduces that run.
